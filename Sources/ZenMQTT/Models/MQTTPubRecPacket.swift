...
final class MQTTPubRecPacket: MQTTPacket, @unchecked Sendable {
    let messageID: UInt16
    let protocolVersion: MQTTProtocolVersion
    let reasonCode: MQTTPubRecReasonCode
    ...
    init(
        ...
        super.init(header: MQTTPacketFixedHeader(packetType: .pubRec, flags: 0))
    }
    ...
    init(header: MQTTPacketFixedHeader, networkData: Data, protocolVersion: MQTTProtocolVersion) {
        ...
        if protocolVersion == .v500, networkData.count > 2 {
            self.reasonCode = MQTTPubRecReasonCode(rawValue: networkData[2]) ?? .unspecifiedError
